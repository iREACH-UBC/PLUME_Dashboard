...
def get_modbus_data(ip, port, enable_pollutant, holding_regs, disabled_behaviour):
    ...
    while True:
        ...
        if enable_pollutant['wcpc']:
            ...
            wcpc_data = dict(concentration=wcpc_decoder.decode_32bit_float(), time2=(datetime.datetime.now()).strftime("%H:%M:%S"))
        elif disabled_behaviour == 'random':
        ...
        if enable_pollutant['co2']:
            ...
            co2_data = dict(CO2=co2_decoder.decode_32bit_float(), time5=(datetime.datetime.now()).strftime("%H:%M:%S"))
        elif disabled_behaviour == 'random':
        ...
        if enable_pollutant['co']:
            ...
            co_data = dict(CO=co_decoder.decode_32bit_float(), time4=(datetime.datetime.now()).strftime("%H:%M:%S"))
        elif disabled_behaviour == 'random':
